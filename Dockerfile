...
RUN			echo "deb http://nginx.org/packages/mainline/debian/ jessie nginx" | tee -a /etc/apt/source.list.d/nginx.list
RUN			echo "deb-src http://nginx.org/packages/mainline/debian/ jessie nginx" | tee -a /etc/apt/source.list.d/nginx.list
RUN			apt-get update && \
